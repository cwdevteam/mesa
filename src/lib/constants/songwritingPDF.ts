...
const getRightsAndDutiesVote = (votePercentage: number) => {
  const rightsAndDutiesVote = {
    ...
    paragraphs: [
      ...
      {
        text: `The parties co-owning the Musical Work shall refrain from performing legally relevant acts without the written authorization of the <span class="underline">${votePercentage}</span>% of the ownership interest, such as but not limited to the following.`,
        children: [
          {
...
export const getSongwritingPDFHeadings = ({
  type,
  adminName,
  votePercentage,
}: PDFHeadingProps) => {
